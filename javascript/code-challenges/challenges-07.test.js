...
const removeThree = (idx, arr) => {
  arr.splice(idx, 3);
  return arr;
};  // I don't understand why this passed if it didn't pass as return arr.splice(idx, 3);
...
const joinArray = (arr) => {
  return arr.join(' ');  // the argument being passed is the 'delimiter' or the string separating what was once the array cells.
};
...
const howMuchPencil = (str) => {
  let result = [str];  // I think?
  for (let i = 1; i <= str.length; i++) {
    result.push(str.substring(i));  // substring is pretty cool
    }
  return result;
};
...
//   let result = [];
//   let recipeArray = recipe.ingredients;

//   for (let i = 0; i < recipeArray.length; i++) {
//     let ingr = recipeArray[i];
//     let idx = ingr.indexOf(' '); // don't really get how indexOf works very well
//     let name = ingr.slice(idx + 1);
//     result.push(name);
//   }
//   return result;
// };
...
const listFoods = (recipe) => {
  let result = [];
  let arrayRecipes = recipe.ingredients;

  for(let i in arrayRecipes){
    result.push(arrayRecipes[i].slice(arrayRecipes[i].indexOf(' ', 3)).slice(1));
  }
  return result;
};  // this was me trying again a different way
