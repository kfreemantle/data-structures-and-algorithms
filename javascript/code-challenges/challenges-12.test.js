...
const validatePin = (pin) => {
  // I don't really understand how the ^ character works in this instance.  the documentation has me reading it pretty backwards
  let regex = /^\d{4}$/gm;
...
const validateWord = (word) => {
  // use the ^ followed by inclusion of all letters upper/lowercase, then the desired length in curly braces then gm
  // it passes with both A-z and A-Za-z but it seemed like it SHOULDN'T have passed with just A-z
  let regex = /^[A-z]{5,10}$/gm;
...
const hasNumber = (string) => {
  // do we need to use 0-9 to account for all numbers or is there a better syntax?
  let regex =/[A-z]+\d/gm;
...
const validateEmail = (email) => {
  let regex = /^[A-z]+(\.{0,1}([A-z0-9]+))?(@[A-z]+\.(com|net|org))$/gm;
...
const validatePhoneNumber = (phoneNumber) => {
  // there has to be a shorter way to do this, right?
  let regex = /^\([0-9]{3}\)|^[0-9]{3}-?[0-9]{3}(\s|-)?[0-9]{4}$|^[0-9]{3}(\s)?[0-9]{3}(\s|-)?[0-9]{4}$/gm;
