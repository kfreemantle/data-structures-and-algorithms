...
const typeNum = (arr) => {
  return arr.filter( (arrayElement) => typeof (arrayElement) === 'number');
};
...
const containsAnd = (arr) => {
  return arr.filter( (arrayElement) => arrayElement.includes('and'));
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let processedArray = arr.filter( firstArrayItems => {
    return !(forbiddenValues.includes(firstArrayItems));
  });
