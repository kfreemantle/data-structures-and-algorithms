...
const screenForNames = (arr) => {
  // if it's the beginning or end of 'title' followed by a space we shouldn't need to call out the titles all specifically
  let regex = /^[MD][rs]+.\s\w/gm;
  return arr.filter(greeting => greeting.match(regex));
};
...
let biggerThanLuke = (arr) => {
  // if we just care if they're bigger than luke we should just need to map if we don't care about ordering
...
const detectTicTacToeWin = (board) => {
  // I am stymied on this one and now it's getting locked in a few hours
};
